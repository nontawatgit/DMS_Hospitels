...
$num = mysqli_num_rows($result1);
		...
		if($num > 0){
			...
			$sql = ("INSERT INTO tb_user(fname, lname, fullname, email, idcard, tel, birthday, sex, hn_id, user_leve, weight, height, BMI, province, hi) 
					VALUES('$fname', '$lname', '$fullname', '$email', '$idcard', '$tel', '$birthday', '$sex', '$hn_id', 'u', '$weight', '$height', '$BMI', '$province', '$hi')");
			$q = mysqli_query($conn,$sql);
			
			$sql_tb_wh = ("INSERT INTO tb_wh(userid_wh, weight_wh, height_wh, BMI_wh, date_time_wh, idcard) 
			VALUES('$userid_wh','$weight', '$height', '$BMI', '$dt', '$idcard')");
